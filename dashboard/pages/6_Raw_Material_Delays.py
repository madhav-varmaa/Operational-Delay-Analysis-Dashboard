...
bar_mat_selection= df_selection.groupby(["MATERIAL"])["EFF_DURATION"].mean().reset_index()

fig_bar_mat = px.bar(bar_mat_selection, x="MATERIAL", y="EFF_DURATION", title="DELAYS BY RAW MATERIALS")
...
fig_mat_pie  = px.pie(df_selection, names='MATERIAL',
                           values='EFF_DURATION',
                           color='MATERIAL',  # Specify the color column
                           color_discrete_sequence=px.colors.qualitative.Plotly,  #built=in color function of plotly
                           title='DELAYS BY RAW MATERIALS')
