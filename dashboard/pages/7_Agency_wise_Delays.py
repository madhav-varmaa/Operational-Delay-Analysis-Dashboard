...
bar_agency_selection= df_selection.groupby(["AGENCY_CODE"])["EFF_DURATION"].mean().reset_index()

fig_bar_agency = px.bar(bar_agency_selection, x="AGENCY_CODE", y="EFF_DURATION", title="DELAYS BY AGENCY")

fig_bar_agency.update_layout(
    plot_bgcolor="rgba(0,0,0,0)",
    xaxis=(dict(showgrid=False))
)
...
fig_agency_pie  = px.pie(df_selection, names='AGENCY_CODE',
                           values='EFF_DURATION',
                           color='AGENCY_CODE',  # Specify the color column
                           color_discrete_sequence=px.colors.qualitative.Plotly,  #built=in color function of plotly
                           title='DELAYS BY AGENCY')
...
st.plotly_chart(fig_bar_agency, use_container_width=True)
