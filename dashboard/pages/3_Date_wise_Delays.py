...
# ---- SIDEBAR ----
st.sidebar.header("Date wise Delays")
...
delays_by_mon_line = df_selection.groupby(by=["DEL_DATE"])["EFF_DURATION"].mean().reset_index()

fig_mon_delays = px.line(
    delays_by_mon_line,
    x = "DEL_DATE",
    y = "EFF_DURATION",
    title="DELAYS BY Days ",
...
bar_day_selection = df_selection.groupby("DEL_DATE")["EFF_DURATION"].mean().reset_index()

fig_day_bar = px.bar(bar_day_selection, x="DEL_DATE", y="EFF_DURATION", title = "Date wise Delays")
